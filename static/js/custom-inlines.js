...
const addQuitarButton = (prefix) => {
  ...
  let context = `div[data-formset=${prefix}]`;
  // Agrego los botones a los tds correspondientes de todas las tablas
  const tds = $('td[id$=DELETE]', context=context);
  let index = 0;
  for (let td of tds) {
  ...
  const botones = $(tds).find('a.quitar');
  for (let b of botones){
    // cada boton tiene un checkbox asociado que indica que elemento debe ser eliminado.
    const checkbox_btn = $(b).prev('.checkboxinput', context=context); 
    b.addEventListener('click', (e) => {
      e.preventDefault();
      $(checkbox_btn).prop("checked", true);
      $(b).parent().parent().hide(); // ocultamos el renglon "eliminado"
    })
  }
}
...
const bindFormSet = function (context) {
  $('[data-formset]', context).each(function (index, el) {
      $('.checkboxinput', context).hide(); // quita los checkboxes de todas las tablas
      const prefix = inlineFormset($(el));
      addQuitarButton(prefix); // reemplaza cada checkbox por un boton "Quitar"
  });
};
// Go bitch!
$(document).ready(() => {
